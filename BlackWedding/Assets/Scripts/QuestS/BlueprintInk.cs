...
public class BlueprintInk : MonoBehaviour
{
	public static event Action<Story> OnCreateStory;
	[SerializeField]
	public int storyIndex = 0;
	[SerializeField]
	public ObjectInteraction ObjectInter;
	...
	void Awake()
	{
		...
		RemoveChildren();
		StartStory();
		//ObjectInter = GetComponentInParent<ObjectInteraction>();
		...
		ObjectInter.OnOpenedUIPanel += StartStory;
		ObjectInter.OnOpenedUIPanel += LoadSt;
	}
	...
	void RefreshView()
	{
		...
		else
		{
			//if (EndingManager.Instance != null)
			//{
			//	EndingManager.Instance.MarkCompletedConversation(ObjectInter.titleOfNPC);
			//	EndingManager.Instance.DetermineTheEnding();

			//}
			ActivateQuestObj();
			//ObjectInter.Perform();
			ObjectInter.CloseUIPanel();
		}
	...
	void ActivateQuestObj()
	{
		...
		{
			if (storyIndex == storyIndexToActivateQuestObj && ObjectInter.quest == QuestManager.Instance.quests[0])
			{
				QuestManager.Instance.currentQuest.MakeObjInteractable();
			}

			if (storyIndex == storyIndexToActivateSecondQuestObj && ObjectInter.quest == QuestManager.Instance.quests[QuestManager.Instance.quests.Length - 1])
			{
